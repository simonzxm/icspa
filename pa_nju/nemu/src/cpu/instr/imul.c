...
make_instr_func(imul_rm2r_v) {
    ...
    operand_read(&r);
    operand_read(&rm);
    r.val = alu_imul(sign_ext(rm.val, data_size), sign_ext(r.val, data_size),
                     data_size);
    operand_write(&r);
...
make_instr_func(imul_irm2r_v) {
    ...
    operand_read(&rm);
    operand_read(&imm);
    r.val = alu_imul(sign_ext(imm.val, data_size), sign_ext(rm.val, data_size),
                     data_size);
    operand_write(&r);
...
make_instr_func(imul_i8rm2r_v) {
    ...
    operand_read(&imm);
    r.val =
        alu_imul(sign_ext(imm.val, 8), sign_ext(rm.val, data_size), data_size);
    operand_write(&r);
