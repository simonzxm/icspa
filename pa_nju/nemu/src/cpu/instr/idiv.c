...
make_instr_func(idiv_rm2a_b) {
    ...
    operand_read(&rm);
    int32_t quotient =
        alu_idiv(sign_ext_64(rm.val, 8), sign_ext_64(ax.val, 16), 8);
    int32_t remainder =
        alu_imod(sign_ext_64(rm.val, 8), sign_ext_64(ax.val, 16));
    OPERAND ah, al;
...
make_instr_func(idiv_rm2a_v) {
    ...
    int32_t remainder = 0;
    if (data_size == 16) {
        quotient = alu_idiv(sign_ext_64(rm.val, 16),
                            sign_ext_64((d.val << 16) | a.val, 32), data_size);
        remainder = alu_imod(sign_ext_64(rm.val, 16),
                             sign_ext_64((d.val << 16) | a.val, 32));
