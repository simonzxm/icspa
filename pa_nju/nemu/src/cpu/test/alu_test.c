...
void alu_test_add() {
    ...
    if (get_ref())
        printf("\e[0;31mYou have used reference implementations, DO NOT submit "
               "this version!\e[0m\n");
}
...
void alu_test_imul() {
    ...
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
            ...
            fflush(stdout);
            assert((int32_t)res == (int32_t)((res_asm_a & 0xffff) |
                                             ((res_asm_d & 0xffff) << 16)));
        }
    }
    ...
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
            ...
            temp_d = res_asm_d;
            fflush(stdout);
            assert((int64_t)res == (int64_t)((temp_a & 0xffffffff) |
                                             ((temp_d & 0xffffffff) << 32)));
        }
    }
