...
paddr_t page_walk(laddr_t laddr) {
    ...
    /*if(!pde.present) {
        Log("eip = %x, lnaddr = %x, pdir_base = %x, pde = %x", cpu.eip, lnaddr,
    pdir_base, pde.val); assert(0);
    }*/
    assert(pde.present);
    ...
    /*if(!pte.present) {
        Log("eip = %x, lnaddr = %x, pt_base = %x, pte = %x", cpu.eip, lnaddr,
    pt_base, pte.val); assert(0);
    }*/
    assert(pte.present);
