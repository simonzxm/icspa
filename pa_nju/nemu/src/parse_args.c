...
static struct alu_operation_map {
    ...
    void (*alu_test_func)();
    void (*alu_score_func)();
} alu_operation_list[] = {{"add", alu_test_add, __score_alu_test_add},
...
#define NR_ALU_OPERATION_TEST                                                  \
    (sizeof(alu_operation_list) / sizeof(alu_operation_list[0]))
...
#define NR_FPU_OPERATION_TEST                                                  \
    (sizeof(fpu_operation_list) / sizeof(fpu_operation_list[0]))
...
static bool check_for_arg_validity() {
    ...
    if (0 == numTargets) {
        printf("\n\nError: specify a target: [--test-reg, --test-alu, "
               "--test-fpu, --testcase, --test-game, --test-expr-score]\n\n");
        return false;
    }
    if (1 < numTargets) {
        printf(
            "\n\nError: only one target is allowed: [--test-reg, --test-alu, "
            "--test-fpu, --testcase,  --test-game, --test-expr-score]\n\n");
        return false;
    }
