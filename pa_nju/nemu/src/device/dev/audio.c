...
/*
 * The implementation of the audio device is determined by whether we are
 * executing NEMU locally in a virtual machine with GUI or visiting the host
 * running nemu from a remote client via ssh with X11 forwarding.
 */

static bool audio_active = false;

#ifdef PA_ONLINE
// We are visiting the host running NEMU from a remote client, and we have the
// teaching assistant web service. The audio device just notifies the web server
// to keep playing the audio record. This is just a trick we used to make it
// possible for NEMU to play audio over the connections. Refer to the code below
// for a proper implementation of a simulated sound card.
...
#else
// We are executing NEMU in a vm with Debian-32 as guest OS and with GUI desktop
// environment like MATE installed.
...
make_pio_handler(audio_io_handler) {
    if (port == AUDIO_CTL && is_write) {
        ...
        if (cmd != audio_state) {
            // while(cmd == 0 && audio_chunk_ready[audio_chunk_play_idx] ==
            // false) { 	SDL_Delay(1);
            // }
        ...
        data_len =
            vaddr_read(audio_addr, SREG_DS, 4); // first 4 bytes as data length
...
void fill_audio(void *udata, Uint8 *stream, int len) {
    ...
    if (audio_len[audio_chunk_play_idx] > 0 &&
        audio_chunk_ready[audio_chunk_play_idx]) { // do nothing
    } else {
        ...
        audio_chunk_play_idx =
            (audio_chunk_play_idx + 1) % 2; // equal to old fill idx
        audio_pos =
            audio_chunk[audio_chunk_play_idx]; // should be already ready
        SDL_UnlockMutex(
            audio_mutex[audio_chunk_fill_idx]); // unlock the old play idx
        switch_buf = true;
    }

    len =
        (len > audio_len[audio_chunk_play_idx] ? audio_len[audio_chunk_play_idx]
                                               : len);
    SDL_MixAudio(stream, audio_pos, len,
                 SDL_MIX_MAXVOLUME); // when using local SDL, write to socket
                                     // when configured for web
    audio_pos += len;
