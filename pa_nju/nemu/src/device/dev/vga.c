...
enum {Horizontal_Total_Register, End_Horizontal_Display_Register,
    ...
    Start_Address_High_Register, Start_Address_Low_Register,
Cursor_Location_High_Register, Cursor_Location_Low_Register,
Vertical_Retrace_Start_Register, Vertical_Retrace_End_Register,
Vertical_Display_End_Register, Offset_Register, Underline_Location_Register,
Start_Vertical_Blanking_Register, End_Vertical_Blanking,
        ...
        *color_ptr++ = (uint8_t)(read_io_port(port, 1) << 2);
        if ((((void *)color_ptr - (void *)&screen->format->palette->colors) &
            ...
            if ((void *)color_ptr == (void *)&palette[256]) {
                SDL_SetPalette(real_screen, SDL_LOGPAL | SDL_PHYSPAL,
                               (void *)&palette, 0, 256);
...
make_pio_handler(vga_crtc_io_handler) {
    if (port == VGA_CRTC_INDEX && is_write) {
        ...
        vga_crtc_regs[read_io_port(VGA_CRTC_INDEX, 1)] =
            (uint8_t)read_io_port(VGA_CRTC_DATA, 1);
    }
}

void vga_init() {
    // vga_dac_port_base = add_pio_map(VGA_DAC_WRITE_INDEX, 2,
    // vga_dac_io_handler); vga_crtc_port_base = add_pio_map(VGA_CRTC_INDEX, 2,
    // vga_crtc_io_handler);
