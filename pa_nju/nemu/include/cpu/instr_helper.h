...
void print_asm_2(char *instr, char *suffix, uint8_t len, OPERAND *opr_1,
                 OPERAND *opr_2);
void print_asm_3(char *instr, char *suffix, uint8_t len, OPERAND *opr_1,
                 OPERAND *opr_2, OPERAND *opr_3);
...
#define make_instr_impl_1op(inst_name, src_type, suffix)                       \
    make_instr_func(concat5(inst_name, _, src_type, _, suffix)) {              \
        int len = 1;                                                           \
        concat(decode_data_size_, suffix) concat3(decode_operand, _, src_type) \
            print_asm_1(#inst_name,                                            \
                        opr_src.data_size == 8                                 \
                            ? "b"                                              \
                            : (opr_src.data_size == 16 ? "w" : "l"),           \
                        len, &opr_src);                                        \
        instr_execute_1op();                                                   \
        return len;                                                            \
    }
...
// macro for generating the implementation of an instruction with one operand
// and condition for jcc and setcc, the opcode type are always fixed so it will
// not appear in the function name
#define make_instr_impl_1op_cc(inst_name, src_type, suffix, cc)                \
    make_instr_func(concat4(inst_name, cc, _, suffix)) {                       \
        ...
        if (concat(condition_, cc)) instr_execute_1op_cc_pass();               \
        else instr_execute_1op_cc_fail();                                      \
        return len;                                                            \
    }
...
#define make_instr_impl_2op_cc(inst_name, src_type, dest_type, suffix, cc)     \
    make_instr_func(concat7(concat(inst_name, cc), _, src_type, 2, dest_type,  \
                            _, suffix)) {                                      \
        int len = 1;                                                           \
        concat(decode_data_size_, suffix)                                      \
            concat3(decode_operand, _, concat3(src_type, 2, dest_type))        \
                print_asm_2(#inst_name, #cc, len, &opr_src, &opr_dest);        \
        if (concat(condition_, cc)) instr_execute_2op_cc_pass();               \
        else instr_execute_2op_cc_fail();                                      \
        return len;                                                            \
    }
...
#define decode_operand_rm len += modrm_rm(eip + 1, &opr_src);
...
#define decode_operand_r2rm len += modrm_r_rm(eip + 1, &opr_src, &opr_dest);

#define decode_operand_rm2r len += modrm_r_rm(eip + 1, &opr_dest, &opr_src);
...
// conditions
// possible condition: e, a, ae, b, be, o, p, s , ne, na, no, np, ns, g, ge, l,
// le, ecxz

#define condition_e (cpu.eflags.ZF)

#define condition_a (!cpu.eflags.CF && !cpu.eflags.ZF)

#define condition_ae (!cpu.eflags.CF)

#define condition_b (cpu.eflags.CF)

#define condition_be (cpu.eflags.CF || cpu.eflags.ZF)

#define condition_o (cpu.eflags.OF)

#define condition_p (cpu.eflags.PF)

#define condition_s (cpu.eflags.SF)

#define condition_ne (!cpu.eflags.ZF)

#define condition_na (cpu.eflags.CF || cpu.eflags.ZF)

#define condition_no (!cpu.eflags.OF)

#define condition_np (!cpu.eflags.PF)

#define condition_ns (!cpu.eflags.SF)

#define condition_g (!cpu.eflags.ZF && (cpu.eflags.SF == cpu.eflags.OF))

#define condition_ge (cpu.eflags.SF == cpu.eflags.OF)

#define condition_l (cpu.eflags.SF != cpu.eflags.OF)

#define condition_le (cpu.eflags.ZF || (cpu.eflags.SF != cpu.eflags.OF))

#define condition_ecxz (cpu.ecx == 0)

#define condition_c (cpu.eflags.CF)
