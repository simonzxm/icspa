...
static void set_intr(GateDesc *ptr, uint32_t selector, uint32_t offset,
                     uint32_t dpl) {
...
static void set_trap(GateDesc *ptr, uint32_t selector, uint32_t offset,
                     uint32_t dpl) {
    ptr->offset_15_0 = offset & 0xFFFF;
    ptr->segment = selector;
    ptr->pad0 = 0;
...
void init_idt() {
    int i;
    for (i = 0; i < NR_IRQ; i++) {
        set_trap(idt + i, SEG_KERNEL_CODE << 3, (uint32_t)irq_empty,
                 DPL_KERNEL);
    }
    ...
    set_intr(idt + 32 + 0, SEG_KERNEL_CODE << 3, (uint32_t)irq0,
             DPL_KERNEL); // timer
    set_intr(idt + 32 + 1, SEG_KERNEL_CODE << 3, (uint32_t)irq1,
             DPL_KERNEL); // keyboard
    set_intr(idt + 32 + 2, SEG_KERNEL_CODE << 3, (uint32_t)irq2,
             DPL_KERNEL); // audio, experimental
    set_intr(idt + 32 + 14, SEG_KERNEL_CODE << 3, (uint32_t)irq14,
             DPL_KERNEL); // ide
