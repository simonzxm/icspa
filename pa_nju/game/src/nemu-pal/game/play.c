...
--*/
{
    ...
    if (fTrigger) {
        ...
        if (gpGlobals->fEnteringScene) {
            ...
            gpGlobals->g.rgScene[i].wScriptOnEnter = PAL_RunTriggerScript(
        ...
        //
        for (wEventObjectID = gpGlobals->g.rgScene[gpGlobals->wNumScene - 1]
                                  .wEventObjectIndex +
                              1;
             wEventObjectID <=
            ...
            if (p->sState < 0) {
                ...
                //
                if (abs(PAL_X(gpGlobals->viewport) +
                        PAL_X(gpGlobals->partyoffset) - p->x) +
                        abs(PAL_Y(gpGlobals->viewport) +
                    ...
                    if (p->nSpriteFrames) {
                        //
                        // The sprite has multiple frames. Try to adjust the
                        // direction.
                        //
                        int xOffset, yOffset;
                        ...
                        if (xOffset > 0) {
                            p->wDirection =
                                ((yOffset > 0) ? kDirEast : kDirNorth);
                        } else {
                            p->wDirection =
                                ((yOffset > 0) ? kDirSouth : kDirWest);
                        }
    ...
    for (wEventObjectID =
        ...
        if (p->sState > 0 && p->sVanishTime == 0) {
            ...
            if (wScriptEntry != 0) {
                p->wAutoScript =
                    PAL_RunAutoScript(wScriptEntry, wEventObjectID);
                if (gpGlobals->fEnteringScene || gpGlobals->fGameStart) {
                    //
                    // Don't go further on scene switching
                    //
        ...
        if (fTrigger && p->sState >= kObjStateBlocker && p->wSpriteNum != 0 &&
            abs(p->x - PAL_X(gpGlobals->viewport) -
...
--*/
{
    ...
    while (TRUE) {
        ...
        if (!(gpGlobals->g.rgObject[wObject].item.wFlags &
              kItemFlagApplyToAll)) {
            //
            ...
            while (TRUE) {
                ...
                //
                // Remove the item if the item is consuming and the script
                // succeeded
                //
                if ((gpGlobals->g.rgObject[wObject].item.wFlags &
                     kItemFlagConsuming) &&
...
--*/
{
    ...
    for (i = 0; i < 13; i++) {
        ...
        //
        for (k = gpGlobals->g.rgScene[gpGlobals->wNumScene - 1]
                     .wEventObjectIndex;
            ...
            if (p->sState <= 0 || p->wTriggerMode >= kTriggerTouchNear ||
                p->wTriggerMode * 6 - 4 < i || dx != ex || dy != ey ||
                dh != eh) {
                continue;
            }
            ...
            if (p->nSpriteFrames * 4 > p->wCurrentFrameNum) {
                ...
                p->wDirection =
                    (gpGlobals->wPartyDirection + 2) % 4; // face the party

                for (l = 0; l <= gpGlobals->wMaxPartyMemberIndex; l++) {
                    //
                    // All party members should face the event object
                    //
                    gpGlobals->rgParty[l].wFrame =
                        gpGlobals->wPartyDirection * 3;
                }
