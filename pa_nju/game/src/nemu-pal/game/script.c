...
/*++
  Purpose:
    ...
    Make the specified event object walk to the map position specified by (x, y,
h) at the speed of iSpeed.
...
--*/
{
    ...
    xOffset = x * 32 + h * 16 - PAL_X(gpGlobals->viewport) -
              PAL_X(gpGlobals->partyoffset);
    yOffset = y * 16 + h * 8 - PAL_Y(gpGlobals->viewport) -
...
static VOID PAL_PartyRideEventObject(WORD wEventObjectID, INT x, INT y, INT h,
                                     INT iSpeed)
/*++
  Purpose:
...
--*/
{
    ...
    while (xOffset != 0 || yOffset != 0) {
        ...
        // Move the viewport
        //
        gpGlobals->viewport = PAL_XY(PAL_X(gpGlobals->viewport) + dx,
                                     PAL_Y(gpGlobals->viewport) + dy);
...
static VOID PAL_MonsterChasePlayer(WORD wEventObjectID, WORD wSpeed,
                                   WORD wChaseRange, BOOL fFloating)
/*++
  Purpose:
        ...
        if (abs(x) + abs(y) * 2 < wChaseRange * 32 * gpGlobals->wChaseRange) {
            ...
            if (fFloating) {
                ...
                for (l = 0; l < 4; l++) {
                    ...
                    if (PAL_CheckObstacle(PAL_XY(pEvtObj->x, pEvtObj->y), FALSE,
                                          0)) {
                        pEvtObj->x = prevx;
                        pEvtObj->y = prevy;
                    }
...
--*/
{
...
#endif
        " ",
        "\xA5\xBB\xB5\x7B\xA6\xA1\xAC\x4F\xA6\xDB\xA5\xD1\xB3\x6E\xC5\xE9\xA1"
        "\x41\xAB\xF6\xB7\xD3"
        " GNU General",
    ...
    while (rgszStrings[i][0] != '\0') {
        PAL_DrawText(rgszStrings[i], PAL_XY(25, 20 + i * 16), DESCTEXT_COLOR,
                     TRUE, FALSE);
        i++;
    }
...
--*/
{
    ...
    case 0x001B:
        ...
        if (pScript->rgwOperand[0]) {
            ...
            //
            // Apply to one player. The wEventObjectID parameter should indicate
            // the player role.
            //
            if (!PAL_IncreaseHPMP(wEventObjectID,
                                  (SHORT)(pScript->rgwOperand[1]), 0)) {
    ...
    case 0x0021:
        ...
        if (pScript->rgwOperand[0]) {
            ...
            //
            g_Battle.rgEnemy[wEventObjectID].e.wHealth -=
                pScript->rgwOperand[1];
        }
        break;
    ...
    case 0x0023:
        ...
        if (pScript->rgwOperand[1] == 0) {
            ...
            if (w != 0) {
                PAL_RemoveEquipmentEffect(iPlayerRole,
    ...
    case 0x0028:
        ...
        if (pScript->rgwOperand[0]) {
            ...
            if (RandomLong(0, 9) >=
                gpGlobals->g.rgObject[w].enemy.wResistanceToSorcery) {
                for (j = 0; j < MAX_POISONS; j++) {
                    if (g_Battle.rgEnemy[wEventObjectID]
                            .rgPoisons[j]
                            .wPoisonID == pScript->rgwOperand[1]) {
                        break;
                    }
                }
    ...
    case 0x0029:
        ...
        if (pScript->rgwOperand[0]) {
            ...
            //
            if (RandomLong(1, 100) >
                PAL_GetPlayerPoisonResistance(wEventObjectID)) {
    ...
    case 0x002E:
        ...
        if (RandomLong(0, i) >=
                gpGlobals->g.rgObject[w].enemy.wResistanceToSorcery &&
            g_Battle.rgEnemy[wEventObjectID]
                    .rgwStatus[pScript->rgwOperand[0]] == 0) {
    ...
    case 0x0038:
        ...
        if (!gpGlobals->fInBattle &&
            ...
            gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wScriptOnTeleport =
                PAL_RunTriggerScript(
                    gpGlobals->g.rgScene[gpGlobals->wNumScene - 1]
                        .wScriptOnTeleport,
                    0xFFFF);
        } else {
            //
    ...
    case 0x0042:
        ...
        PAL_BattleSimulateMagic(i, pScript->rgwOperand[0],
                                pScript->rgwOperand[1]);
        break;
    ...
    case 0x0058:
        //
        // Jump if there is less than the specified number of the specified
    ...
    case 0x0059:
        ...
        //
        if (pScript->rgwOperand[0] > 0 &&
            pScript->rgwOperand[0] <= MAX_SCENES &&
    ...
    case 0x005D:
        ...
        //
        if (!PAL_IsPlayerPoisonedByKind(wEventObjectID,
                                        pScript->rgwOperand[0])) {
            wScriptEntry = pScript->rgwOperand[1] - 1;
        }
        break;
    ...
    case 0x0066:
        ...
        w += RandomLong(0, 4);
        PAL_BattleSimulateMagic((SHORT)wEventObjectID, pScript->rgwOperand[0],
                                w);
        break;
    ...
    case 0x006D:
        ...
        if (pScript->rgwOperand[0]) {
            if (pScript->rgwOperand[1]) {
                gpGlobals->g.rgScene[pScript->rgwOperand[0] - 1]
                    .wScriptOnEnter = pScript->rgwOperand[1];
            }

            if (pScript->rgwOperand[2]) {
                gpGlobals->g.rgScene[pScript->rgwOperand[0] - 1]
                    .wScriptOnTeleport = pScript->rgwOperand[2];
            }
    ...
    case 0x0075:
        ...
        for (i = 0; i < 3; i++) {
            if (pScript->rgwOperand[i] != 0) {
                gpGlobals->rgParty[gpGlobals->wMaxPartyMemberIndex]
                    .wPlayerRole = pScript->rgwOperand[i] - 1;

                g_Battle.rgPlayer[gpGlobals->wMaxPartyMemberIndex]
...
//
    ...
    case 0x0079:
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
            if (gpGlobals->g.PlayerRoles
                    .rgwName[gpGlobals->rgParty[i].wPlayerRole] ==
                pScript->rgwOperand[0]) {
                wScriptEntry = pScript->rgwOperand[1] - 1;
                break;
            }
        ...
        if (pScript->rgwOperand[0] == 0 && pScript->rgwOperand[1] == 0) {
            ...
            do {
                if (pScript->rgwOperand[2] == 0xFFFF) {
                    ...
                    gpGlobals->viewport =
                        PAL_XY(pScript->rgwOperand[0] * 32 - 160,
                    ...
                    gpGlobals->viewport =
                        PAL_XY(PAL_X(gpGlobals->viewport) + x,
                               PAL_Y(gpGlobals->viewport) + y);
        ...
        //
        {
            if (pScript->rgwOperand[0] <=
                    gpGlobals->g.rgScene[gpGlobals->wNumScene - 1]
                        .wEventObjectIndex ||
                pScript->rgwOperand[0] >
                    gpGlobals->g.rgScene[gpGlobals->wNumScene]
                        .wEventObjectIndex) {
                //
            ...
            if (abs(x) + abs(y * 2) < pScript->rgwOperand[1] * 32 + 16) {
                if (pScript->rgwOperand[1] > 0) {
                    //
                    // Change the trigger mode so that the object can be
                    // triggered in next frame
                    //
    ...
    case 0x0083:
        //
        // Jump if event object is not in the specified zone of the current
        // event object
        //
        if (pScript->rgwOperand[0] <=
                gpGlobals->g.rgScene[gpGlobals->wNumScene - 1]
    ...
    case 0x0092:
        ...
        if (gpGlobals->fInBattle) {
            if (pScript->rgwOperand[0] != 0) {
                PAL_BattleShowPlayerPreMagicAnim(pScript->rgwOperand[0] - 1,
                                                 FALSE);
...
//
    ...
    case 0x0098:
        ...
        if (pScript->rgwOperand[0] > 0) {
            ...
            gpGlobals->rgParty[gpGlobals->wMaxPartyMemberIndex + 1]
                .wPlayerRole = pScript->rgwOperand[0];
    ...
    case 0x009E:
        ...
        x = 0;
        w = pScript->rgwOperand[0];
        y = (((SHORT)(pScript->rgwOperand[1]) <= 0)
                 ? 1
                 : (SHORT)pScript->rgwOperand[1]);
    ...
    case 0x009F:
        ...
        if (g_Battle.iHidingTime <= 0 &&
            ...
            g_Battle.rgEnemy[wEventObjectID].e =
                gpGlobals->g
                    .lprgEnemy[gpGlobals->g.rgObject[pScript->rgwOperand[0]]
                                   .enemy.wEnemyID];
...
//
    ...
    default:
        TerminateOnError(
            "SCRIPT: Invalid Instruction at %4x: (%4x - %4x, %4x, %4x)",
...
--*/
{
    ...
    while (wScriptEntry != 0 && !fEnded) {
        ...
        case 0x0007:
            ...
            // Start battle
            //
            i = PAL_StartBattle(pScript->rgwOperand[0],
                                !pScript->rgwOperand[2]);
        ...
        case 0x0009:
            ...
            //
            {
                ...
                for (i = 0;
                     i < (pScript->rgwOperand[0] ? pScript->rgwOperand[0] : 1);
                     i++) {
                    PAL_ProcessEvent();
                    while (SDL_GetTicks() < time) {
                        PAL_ProcessEvent();
        ...
        case 0x003E:
            ...
            //
            PAL_ClearDialog(TRUE);
            PAL_StartDialog(kDialogCenterWindow, (BYTE)pScript->rgwOperand[0],
                            0, FALSE);
            wScriptEntry++;
            break;
        ...
        default:
            PAL_ClearDialog(TRUE);
            wScriptEntry =
                PAL_InterpretInstruction(wScriptEntry, wEventObjectID);
            break;
        }
    }
