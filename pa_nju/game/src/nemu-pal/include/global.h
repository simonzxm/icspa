...
typedef struct tagEVENTOBJECT {
    ...
    USHORT nSpriteFramesAuto; // total number of frames of the sprite, used by
                              // auto script
...
typedef struct tagSCENE {
    ...
    WORD wScriptOnTeleport; // when teleporting out of this scene, execute
                            // script from here
    WORD wEventObjectIndex; // event objects in this scene begins from number
                            // wEventObjectIndex + 1
} SCENE, *LPSCENE;

// object including system strings, players, items, magics, enemies and poison
// scripts.

// system strings and players
typedef struct tagOBJECT_PLAYER {
    WORD wReserved[2];         // always zero
    WORD wScriptOnFriendDeath; // when friends in party dies, execute script
                               // from here
...
typedef struct tagOBJECT_ENEMY {
    ...
    WORD wResistanceToSorcery; // resistance to sorcery and poison (0 min, 10
                               // max)
...
typedef struct tagOBJECT_POISON {
    ...
    WORD wPlayerScript; // script executed when player has this poison (per
                        // round)
...
typedef struct tagPLAYERROLES {
    ...
    WORD rgwElementalResistance[NUM_MAGIC_ELEMENTAL]
                               [MAX_PLAYER_ROLES]; // resistance to elemental
                                                   // magics
...
typedef struct tagGLOBALVARS {
    ...
    PAL_POS partyoffset;
    WORD wLayer;
    WORD wMaxPartyMemberIndex; // max index of members in party (0 to
                               // MAX_PLAYERS_IN_PARTY - 1)
