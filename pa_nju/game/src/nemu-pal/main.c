...
/*++
  ...
  Parameters:
    ...
    [IN]  fFullScreen - TRUE to use full screen mode, FALSE to use windowed
mode.
...
#else
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_CDROM |
                 SDL_INIT_NOPARACHUTE | SDL_INIT_JOYSTICK) == -1)
#endif
    {
...
--*/
{
    ...
    //
    lpBitmapDown =
        SDL_CreateRGBSurface(gpScreen->flags, 320, 200, 8,
...
#else
    SDL_SetPalette(lpBitmapDown, SDL_LOGPAL | SDL_PHYSPAL, VIDEO_GetPalette(),
                   0, 256);
    SDL_SetPalette(lpBitmapUp, SDL_LOGPAL | SDL_PHYSPAL, VIDEO_GetPalette(), 0,
                   256);
#endif
    ...
    while (TRUE) {
        ...
        SDL_SetSurfacePalette(lpBitmapUp, gpScreen->format->palette);
#else
        SDL_SetPalette(lpBitmapDown, SDL_LOGPAL | SDL_PHYSPAL,
                       VIDEO_GetPalette(), 0, 256);
        SDL_SetPalette(lpBitmapUp, SDL_LOGPAL | SDL_PHYSPAL, VIDEO_GetPalette(),
                       0, 256);
#endif
        ...
        if (g_InputState.dwKeyPress & (kKeyMenu | kKeySearch)) {
            ...
            if (dwTime < 15000) {
                ...
                while (dwTime < 15000) {
                    for (i = 0; i < 256; i++) {
                        rgCurrentPalette[i].r =
                            (BYTE)(palette[i].r * dwTime / 15000);
                        rgCurrentPalette[i].g =
                            (BYTE)(palette[i].g * dwTime / 15000);
                        rgCurrentPalette[i].b =
                            (BYTE)(palette[i].b * dwTime / 15000);
                    }
                    VIDEO_SetPalette(rgCurrentPalette);
#if SDL_VERSION_ATLEAST(2, 0, 0)
                    SDL_SetSurfacePalette(lpBitmapDown,
                                          gpScreen->format->palette);
                    SDL_SetSurfacePalette(lpBitmapUp,
                                          gpScreen->format->palette);
#else
