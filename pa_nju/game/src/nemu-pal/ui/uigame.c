...
--*/
{
    ...
    //
    wItemSelected =
        PAL_ReadMenu(NULL, rgMenuItem, 5, wDefaultSlot - 1, MENUITEM_COLOR);
...
--*/
{
    ...
    return (wReturnValue == MENUITEM_VALUE_CANCELLED || wReturnValue == 0)
               ? FALSE
               : TRUE;
}
...
--*/
{
    ...
    //
    wReturnValue =
        PAL_ReadMenu(NULL, rgMenuItem, 2, fEnabled ? 1 : 0, MENUITEM_COLOR);
...
--*/
{
    ...
    // Activate the menu
    //
    wReturnValue = PAL_ReadMenu(NULL, rgMenuItem, 5,
                                gpGlobals->bBattleSpeed - 1, MENUITEM_COLOR);
...
#ifdef PAL_CLASSIC
    wReturnValue =
        PAL_ReadMenu(PAL_SystemMenu_OnItemChange, rgSystemMenuItem, 5,
                     gpGlobals->iCurSystemMenuItem, MENUITEM_COLOR);
#else
    wReturnValue =
        PAL_ReadMenu(PAL_SystemMenu_OnItemChange, rgSystemMenuItem, 6,
                     gpGlobals->iCurSystemMenuItem, MENUITEM_COLOR);
#endif
...
--*/
{
    ...
    while (TRUE) {
        wMagic = PAL_MagicSelectionMenu(gpGlobals->rgParty[w].wPlayerRole,
                                        FALSE, wMagic);
        if (wMagic == 0) {
            break;
        }
        ...
        if (gpGlobals->g.rgObject[wMagic].magic.wFlags & kMagicFlagApplyToAll) {
            gpGlobals->g.rgObject[wMagic].magic.wScriptOnUse =
                PAL_RunTriggerScript(
                    gpGlobals->g.rgObject[wMagic].magic.wScriptOnUse, 0);
            ...
            while (wPlayer != MENUITEM_VALUE_CANCELLED) {
                ...
                while (TRUE) {
                    ...
                    if (g_InputState.dwKeyPress & kKeyMenu) {
                        ...
                        gpGlobals->g.rgObject[wMagic]
                            .magic.wScriptOnUse = PAL_RunTriggerScript(
                            gpGlobals->g.rgObject[wMagic].magic.wScriptOnUse,
                            gpGlobals->rgParty[wPlayer].wPlayerRole);

                        if (g_fScriptSuccess) {
                            gpGlobals->g.rgObject[wMagic]
                                .magic.wScriptOnSuccess = PAL_RunTriggerScript(
                                gpGlobals->g.rgObject[wMagic]
                                    .magic.wScriptOnSuccess,
                                gpGlobals->rgParty[wPlayer].wPlayerRole);
...
--*/
{
    ...
    while (TRUE) {
        wReturnValue =
            PAL_ReadMenu(PAL_InGameMenu_OnItemChange, rgMainMenuItem, 4,
...
--*/
{
    ...
    while (iCurrent >= 0 && iCurrent <= gpGlobals->wMaxPartyMemberIndex) {
        ...
        //
        PAL_DrawText(PAL_GetWord(STATUS_LABEL_EXP), PAL_XY(6, 6),
                     MENUITEM_COLOR, TRUE, FALSE);
        PAL_DrawText(PAL_GetWord(STATUS_LABEL_LEVEL), PAL_XY(6, 32),
                     MENUITEM_COLOR, TRUE, FALSE);
        PAL_DrawText(PAL_GetWord(STATUS_LABEL_HP), PAL_XY(6, 54),
                     MENUITEM_COLOR, TRUE, FALSE);
        PAL_DrawText(PAL_GetWord(STATUS_LABEL_MP), PAL_XY(6, 76),
                     MENUITEM_COLOR, TRUE, FALSE);
        PAL_DrawText(PAL_GetWord(STATUS_LABEL_ATTACKPOWER), PAL_XY(6, 98),
                     MENUITEM_COLOR, TRUE, FALSE);
        PAL_DrawText(PAL_GetWord(STATUS_LABEL_MAGICPOWER), PAL_XY(6, 118),
                     MENUITEM_COLOR, TRUE, FALSE);
        PAL_DrawText(PAL_GetWord(STATUS_LABEL_RESISTANCE), PAL_XY(6, 138),
                     MENUITEM_COLOR, TRUE, FALSE);
        PAL_DrawText(PAL_GetWord(STATUS_LABEL_DEXTERITY), PAL_XY(6, 158),
                     MENUITEM_COLOR, TRUE, FALSE);
        PAL_DrawText(PAL_GetWord(STATUS_LABEL_FLEERATE), PAL_XY(6, 178),
                     MENUITEM_COLOR, TRUE, FALSE);
        ...
        PAL_DrawNumber(
            gpGlobals->g
                .rgLevelUpExp[gpGlobals->g.PlayerRoles.rgwLevel[iPlayerRole]],
        ...
        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH),
                             gpScreen, PAL_XY(65, 58));
        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH),
                             gpScreen, PAL_XY(65, 80));
        ...
        PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxHP[iPlayerRole], 4,
                       PAL_XY(63, 61), kNumColorBlue, kNumAlignRight);
        ...
        PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxMP[iPlayerRole], 4,
                       PAL_XY(63, 83), kNumColorBlue, kNumAlignRight);
...
--*/
{
    ...
    while (TRUE) {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
            ...
            PAL_DrawText(
                PAL_GetWord(gpGlobals->g.PlayerRoles
        ...
        if (i > 0) {
            ...
            PAL_DrawText(PAL_GetWord(wItemToUse), PAL_XY(116, 143),
                         STATUS_COLOR_EQUIPMENT, TRUE, FALSE);
            PAL_DrawNumber(i, 2, PAL_XY(170, 133), kNumColorCyan,
                           kNumAlignRight);
        }
        ...
        while (TRUE) {
            ...
            if (SDL_GetTicks() > dwColorChangeTime) {
                if ((WORD)bSelectedColor + 1 >=
                    (WORD)MENUITEM_COLOR_SELECTED_FIRST +
                ...
                PAL_DrawText(
                    PAL_GetWord(gpGlobals->g.PlayerRoles
                                    .rgwName[gpGlobals->rgParty[wSelectedPlayer]
                                                 .wPlayerRole]),
                    PAL_XY(125, 16 + 20 * wSelectedPlayer), bSelectedColor,
                    FALSE, TRUE);
            }
...
--*/
{
    ...
    PAL_DrawText(PAL_GetWord(BUYMENU_LABEL_CURRENT), PAL_XY(30, 115), 0, FALSE,
                 FALSE);
...
--*/
{
    ...
    for (y = 0; y < i; y++) {
        ...
        PAL_DrawNumber(w, 6, PAL_XY(235, 25 + y * 18), kNumColorCyan,
                       kNumAlignRight);
    }
    ...
    while (TRUE) {
        w = PAL_ReadMenu(PAL_BuyMenu_OnItemChange, rgMenuItem, i, w,
                         MENUITEM_COLOR);
...
--*/
{
    ...
    if (gpGlobals->g.rgObject[wCurrentItem].item.wFlags & kItemFlagSellable) {
        PAL_DrawText(PAL_GetWord(SELLMENU_LABEL_PRICE), PAL_XY(230, 160), 0,
                     FALSE, FALSE);
...
--*/
{
    ...
    PAL_MKFDecompressChunk(bufBackground, 320 * 200,
                           EQUIPMENU_BACKGROUND_FBPNUM, gpGlobals->f.fpFBP);
    ...
    while (TRUE) {
        ...
        for (i = 0; i < MAX_PLAYER_EQUIPMENTS; i++) {
            if (gpGlobals->g.PlayerRoles.rgwEquipment[i][w] != 0) {
                PAL_DrawText(
                    PAL_GetWord(gpGlobals->g.PlayerRoles.rgwEquipment[i][w]),
        ...
        //
        PAL_CreateBox(PAL_XY(2, 95), gpGlobals->wMaxPartyMemberIndex, 2, 0,
                      FALSE);
        ...
        //
        if (wItem != 0) {
            PAL_DrawText(PAL_GetWord(wItem), PAL_XY(5, 70),
                         MENUITEM_COLOR_CONFIRMED, TRUE, FALSE);
            PAL_DrawNumber(PAL_GetItemAmount(wItem), 2, PAL_XY(65, 73),
                           kNumColorCyan, kNumAlignRight);
        }
