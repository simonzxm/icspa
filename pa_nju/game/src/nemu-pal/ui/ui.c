...
--*/
{
    ...
    if (fSaveScreen) {
        ...
        save = SDL_CreateRGBSurface(
            gpScreen->flags, rect.w, rect.h, 8, gpScreen->format->Rmask,
...
#else
        SDL_SetPalette(save, SDL_LOGPAL | SDL_PHYSPAL, VIDEO_GetPalette(), 0,
                       256);
#endif
    ...
    for (i = 0; i < nRows; i++) {
        ...
        for (j = 0; j < nColumns; j++) {
            ...
            PAL_RLEBlitToSurface(rglpBorderBitmap[m][n], gpScreen,
                                 PAL_XY(x, rect.y));
...
--*/
{
    ...
    if (fSaveScreen) {
        ...
        SDL_SetSurfacePalette(save, gpScreen->format->palette);
#else
        SDL_SetPalette(save, SDL_PHYSPAL | SDL_LOGPAL, VIDEO_GetPalette(), 0,
                       256);
#endif
        SDL_BlitSurface(gpScreen, &rect, save, NULL);
...
/*++
  ...
  Return value:
    ...
    Return value of the selected menu item. MENUITEM_VALUE_CANCELLED if
cancelled.
    ...
    while (TRUE) {
        ...
        if (rgMenuItem[wCurrentItem].fEnabled) {
            PAL_DrawText(PAL_GetWord(rgMenuItem[wCurrentItem].wNumWord),
                         rgMenuItem[wCurrentItem].pos, MENUITEM_COLOR_SELECTED,
                         FALSE, TRUE);
        }
        ...
        if (g_InputState.dwKeyPress & (kKeyDown | kKeyRight)) {
            ...
            if (rgMenuItem[wCurrentItem].fEnabled) {
                ...
                //
                PAL_DrawText(PAL_GetWord(rgMenuItem[wCurrentItem].wNumWord),
                             rgMenuItem[wCurrentItem].pos, bLabelColor, FALSE,
                             TRUE);
            } else {
                PAL_DrawText(PAL_GetWord(rgMenuItem[wCurrentItem].wNumWord),
                             rgMenuItem[wCurrentItem].pos,
                             MENUITEM_COLOR_INACTIVE, FALSE, TRUE);
            }
