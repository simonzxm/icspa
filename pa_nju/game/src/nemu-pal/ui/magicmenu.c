...
--*/
{
    ...
    if (gpGlobals->lpObjectDesc == NULL) {
        ...
        PAL_DrawNumber(g_wPlayerMP, 4, PAL_XY(265, 14), kNumColorCyan,
                       kNumAlignRight);
    } else {
        ...
        PAL_DrawNumber(rgMagicItem[g_iCurrentItem].wMP, 4, PAL_XY(15, 14),
                       kNumColorYellow, kNumAlignRight);
        PAL_DrawNumber(g_wPlayerMP, 4, PAL_XY(50, 14), kNumColorCyan,
                       kNumAlignRight);
    }
#else
    wScript = gpGlobals->g.rgObject[rgMagicItem[g_iCurrentItem].wMagic]
                  .item.wScriptDesc;
    line = 0;
    ...
    if (g_InputState.dwKeyPress & kKeySearch) {
        if (rgMagicItem[g_iCurrentItem].fEnabled) {
            ...
            PAL_DrawText(PAL_GetWord(rgMagicItem[g_iCurrentItem].wMagic),
                         PAL_XY(j, k), MENUITEM_COLOR_CONFIRMED, FALSE, TRUE);
...
--*/
{
    ...
    for (i = 0; i < MAX_PLAYER_MAGICS; i++) {
        ...
        if (w != 0) {
            ...
            w = gpGlobals->g.rgObject[rgMagicItem[g_iNumMagic].wMagic]
                    .magic.wFlags;
            if (fInBattle) {
...
--*/
{
    ...
    while (TRUE) {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
            PAL_PlayerInfoBox(PAL_XY(w, 165), gpGlobals->rgParty[i].wPlayerRole,
                              100, TIMEMETER_COLOR_DEFAULT, FALSE);
            w += 78;
        }
