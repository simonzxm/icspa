...
--*/
{
    ...
    PAL_RLEBlitToSurface(
        PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_PLAYERINFOBOX), gpScreen, pos);
    ...
    for (iPartyIndex = 0; iPartyIndex <= gpGlobals->wMaxPartyMemberIndex;
         iPartyIndex++) {
        ...
        for (i = 0; i < MAX_POISONS; i++) {
            ...
            if (w != 0 && gpGlobals->g.rgObject[w].poison.wPoisonLevel <= 3) {
                if (gpGlobals->g.rgObject[w].poison.wPoisonLevel >= wMaxLevel) {
                    ...
                    bPoisonColor =
                        (BYTE)(gpGlobals->g.rgObject[w].poison.wColor);
                }
            }
        }
    }
    ...
    if (bPoisonColor == 0xFF) {
        PAL_RLEBlitToSurface(
            PAL_SpriteGetFrame(gpSpriteUI,
                               SPRITENUM_PLAYERFACE_FIRST + wPlayerRole),
...
//
    ...
    PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH),
                         gpScreen, PAL_XY(PAL_X(pos) + 49, PAL_Y(pos) + 6));
    ...
    PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH),
                         gpScreen, PAL_XY(PAL_X(pos) + 49, PAL_Y(pos) + 22));
...
#else
    PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH),
                         gpScreen, PAL_XY(PAL_X(pos) + 49, PAL_Y(pos) + 14));
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxHP[wPlayerRole], 4,
    ...
    PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH),
                         gpScreen, PAL_XY(PAL_X(pos) + 49, PAL_Y(pos) + 24));
    PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxMP[wPlayerRole], 4,
...
--*/
{
    ...
    if (wSelectedItem != 0xFFFF) {
        if (wSelectedItem != 0) {
            ...
            if (gpGlobals->g.rgObject[wSelectedItem].item.wFlags &
                kItemFlagApplyToAll) {
...
--*/
{
...
#endif
    {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
            ...
            } else if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSlow] >
                       0) {
            ...
            PAL_PlayerInfoBox(PAL_XY(91 + 77 * i, 165), wPlayerRole, w, j,
                              FALSE);
        }
    }
    ...
    if (g_Battle.UI.state != kBattleUIWait) {
        ...
        x = g_rgPlayerPos[gpGlobals->wMaxPartyMemberIndex]
                         [g_Battle.UI.wCurPlayerIndex][0] -
            8;
        y = g_rgPlayerPos[gpGlobals->wMaxPartyMemberIndex]
                         [g_Battle.UI.wCurPlayerIndex][1] -
            74;

        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(gpSpriteUI, i), gpScreen,
                             PAL_XY(x, y));
    }
                    ...
                    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
                        if (g_Battle.rgPlayer[i].flTimeMeter >= 100) {
                            g_Battle.rgPlayer[i].flTimeMeter +=
                                100; // HACKHACK: Prevent the time meter from
                                     // going below 100
        ...
        if (g_InputState.dwKeyPress & kKeyMenu) {
            ...
            if (g_Battle.UI.wSelectedIndex != 0) {
                ...
                while (g_Battle.UI.wSelectedIndex != 0 &&
                       g_Battle.rgEnemy[g_Battle.UI.wSelectedIndex].wObjectID ==
                           0) {
                    g_Battle.UI.wSelectedIndex--;
                }
            }
    ...
    case kBattleUISelectTargetEnemyAll:
        ...
        if (s_iFrame & 1) {
            ...
            for (i = g_Battle.wMaxEnemyIndex; i >= 0; i--) {
                ...
                x -= PAL_RLEGetWidth(PAL_SpriteGetFrame(
    ...
    case kBattleUISelectTargetPlayerAll:
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
            if (g_Battle.UI.wActionType == kBattleActionMagic) {
                w = gpGlobals->g.rgObject[g_Battle.UI.wObjectID]
                        .magic.wMagicNumber;
...
//
    ...
    if (SDL_GetTicks() < g_Battle.UI.dwMsgShowTime) {
        //
        // The text should be shown in a small window at the center of the
        // screen
        //
        PAL_POS pos;
        ...
        g_Battle.UI.dwMsgShowTime =
            SDL_GetTicks() + g_Battle.UI.wNextMsgDuration;
    ...
    for (i = 0; i < BATTLEUI_MAX_SHOWNUM; i++) {
        if (g_Battle.UI.rgShowNum[i].wNum > 0) {
            if ((SDL_GetTicks() - g_Battle.UI.rgShowNum[i].dwTime) /
                    BATTLE_FRAME_TIME >
                10) {
                ...
                PAL_DrawNumber(g_Battle.UI.rgShowNum[i].wNum, 5,
                               PAL_XY(PAL_X(g_Battle.UI.rgShowNum[i].pos),
                                      PAL_Y(g_Battle.UI.rgShowNum[i].pos) -
                                          (SDL_GetTicks() -
                                           g_Battle.UI.rgShowNum[i].dwTime) /
                                              BATTLE_FRAME_TIME),
