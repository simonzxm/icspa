...
static const char g_rgszAdditionalWords[][WORD_LENGTH + 1] = {
    {0xBE, 0xD4, 0xB0, 0xAB, 0xB3, 0x74, 0xAB, 0xD7, 0x00, 0x00,
     0x00}, // Battle Speed
...
--*/
{
    ...
    while (*lpszText) {
        ...
        if (*lpszText & 0x80) {
            ...
            //
            wChar =
                SWAP16(((LPBYTE)lpszText)[0] | (((LPBYTE)lpszText)[1] << 8));
            if (fShadow) {
                PAL_DrawCharOnSurface(wChar, gpScreen,
                                      PAL_XY(rect.x + 1, rect.y + 1), 0);
                PAL_DrawCharOnSurface(wChar, gpScreen,
                                      PAL_XY(rect.x + 1, rect.y), 0);
            }
            PAL_DrawCharOnSurface(wChar, gpScreen, PAL_XY(rect.x, rect.y),
                                  bColor);
            lpszText += 2;
            rect.x += 16;
            urect.w += 16;
...
--*/
{
    ...
    case kDialogUpper:
        if (iNumCharFace > 0) {
            ...
            //
            if (PAL_MKFReadChunk(buf, 16384, iNumCharFace, gpGlobals->f.fpRGM) >
                0) {
...
--*/
{
    ...
    //
    pCurrentPalette =
        PAL_GetPalette(gpGlobals->wNumPalette, gpGlobals->fNightPalette);
    ...
    if (g_TextLib.bDialogPosition != kDialogCenterWindow &&
        ...
        //
        LPCBITMAPRLE p =
            PAL_SpriteGetFrame(g_TextLib.bufDialogIcons, g_TextLib.bIcon);
        if (p != NULL) {
            SDL_Rect rect;
...
//
        ...
        if (g_TextLib.nCurrentDialogLine == 0 &&
            g_TextLib.bDialogPosition != kDialogCenter &&
            (((BYTE)lpszText[len - 1] == 0x47 &&
              (BYTE)lpszText[len - 2] == 0xA1) ||
             ((BYTE)lpszText[len - 1] == 0xBA &&
              (BYTE)lpszText[len - 2] == 0xA3) ||
             ((BYTE)lpszText[len - 1] == 0xC3 &&
              (BYTE)lpszText[len - 2] == 0xA1) ||
             ((BYTE)lpszText[len - 1] == ':'))) {
            //
            // name of character
            //
            PAL_DrawText(lpszText, g_TextLib.posDialogTitle,
            ...
            while (lpszText != NULL && *lpszText != '\0') {
                ...
                default:
                    ...
                    PAL_DrawText(text, PAL_XY(x, y),
                                 g_TextLib.bCurrentFontColor, TRUE, TRUE);
                    x += ((text[0] & 0x80) ? 16 : 8);
...
--*/
{
    ...
    // which doesn't have a "start dialog" instruction before showing the
    // dialog.
    //
    g_TextLib.posDialogTitle = PAL_XY(12, 8);
    g_TextLib.posDialogText = PAL_XY(44, 26);
