...
#define PAL_DOALLOCATE(fp, num, type, lptype, ptr, n)                          \
    {                                                                          \
        ...
        if (ptr == NULL) {                                                     \
            TerminateOnError(                                                  \
                "PAL_InitGlobalGameData(): Memory allocation error!");         \
        }                                                                      \
    }
    ...
    if (gpGlobals->g.lprgEventObject == NULL) {
        ...
        PAL_DOALLOCATE(gpGlobals->f.fpDATA, 6, LEVELUPMAGIC_ALL,
                       LPLEVELUPMAGIC_ALL, gpGlobals->g.lprgLevelUpMagic,
                       gpGlobals->g.nLevelUpMagic);
...
--*/
{
    ...
    memset(gpGlobals->rgPoisonStatus, 0, sizeof(gpGlobals->rgPoisonStatus));
    memcpy(gpGlobals->rgInventory, s.rgInventory,
           sizeof(gpGlobals->rgInventory));
...
--*/
{
    ...
    memcpy(s.rgInventory, gpGlobals->rgInventory,
           sizeof(gpGlobals->rgInventory));
...
--*/
{
    ...
    for (i = 0; i < MAX_POISONS; i++) {
        ...
        if (w >= 99) {
            //
            // Ignore poisons which has a level of 99 (usually effect of
            // equipment)
            //
            continue;
        }
...
--*/
{
    ...
    for (i = 0; i <= MAX_PLAYER_EQUIPMENTS; i++) {
        if (gpGlobals->rgEquipmentEffect[i].rgwSpriteNumInBattle[wPlayerRole] !=
            0) {
            w = gpGlobals->rgEquipmentEffect[i]
                    .rgwSpriteNumInBattle[wPlayerRole];
        }
    }
...
--*/
{
    ...
    for (i = 0; i <= MAX_PLAYER_EQUIPMENTS; i++) {
        if (gpGlobals->rgEquipmentEffect[i].rgwCooperativeMagic[wPlayerRole] !=
            0) {
            w = gpGlobals->rgEquipmentEffect[i]
                    .rgwCooperativeMagic[wPlayerRole];
        }
    }
