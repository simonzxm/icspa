...
static unsigned int get_bits(const void *src, unsigned int *bitptr,
                             unsigned int count) {
...
static unsigned short get_loop(const void *src, unsigned int *bitptr,
    ...
    if (get_bits(src, bitptr, 1)) return header->CodeCountTable[0];
    else {
        unsigned int temp = get_bits(src, bitptr, 2);
        if (temp)
            return get_bits(src, bitptr,
                            header->CodeCountCodeLengthTable[temp - 1]);
        else return header->CodeCountTable[1];
    }
}
...
static unsigned short get_count(const void *src, unsigned int *bitptr,
    ...
    if ((temp = get_bits(src, bitptr, 2)) != 0) {
        ...
        else return SWAP16(header->LZSSRepeatTable[temp]);
    } else return SWAP16(header->LZSSRepeatTable[0]);
}
...
INT Decompress(LPCVOID Source, LPVOID Destination, INT DestSize) {
    ...
    do {
        ...
        if ((node = root =
                 (TreeNode *)malloc(sizeof(TreeNode) * (tree_len + 1))) == NULL)
            return -1;
        root[0].leaf = 0;
        root[0].value = 0;
        root[0].left = root + 1;
        ...
        for (i = 1; i <= tree_len; i++) {
            ...
            if (root[i].leaf) root[i].left = root[i].right = NULL;
            else {
    ...
    for (i = 0; i < SWAP16(hdr->BlockCount); i++) {
        ...
        for (;;) {
            ...
            while (loop--) {
                node = root;
                for (; !node->leaf;) {
                    if (get_bits(src, &bitptr, 1)) node = node->right;
                    else node = node->left;
                }
                *dest++ = node->value;
            }
            ...
            while (loop--) {
                ...
                pos = get_bits(src, &bitptr,
                               header->LZSSOffsetCodeLengthTable[pos]);
                while (count--) {
                    *dest = *(dest - pos);
                    dest++;
                }
...
static unsigned char data1[0x100] = {
    ...
    0x18, 0x01, 0x04, 0x00};
static unsigned char data2[0x10] = {0x08, 0x05, 0x06, 0x04, 0x07, 0x05,
...
static void adjust_tree(Tree tree, unsigned short value) {
    ...
    while (node->value != 0x280) {
        ...
        if (temp != node) {
            ...
            } else tree.list[node->value] = temp;
            if (temp->value > 0x140) {
                temp->left->parent = node;
                temp->right->parent = node;
            } else tree.list[temp->value] = node;
            tmp = *node;
            *node = *temp;
            *temp = tmp;
            node = temp;
...
static int build_tree(Tree *tree) {
    ...
    TreeNode **list;
    TreeNode *node;
    if ((tree->list = list = (TreeNode **)malloc(sizeof(TreeNode *) * 321)) ==
        NULL)
        return 0;
    if ((tree->node = node = (TreeNode *)malloc(sizeof(TreeNode) * 641)) ==
        NULL) {
        free(list);
        return 0;
    }
