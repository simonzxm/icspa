...
--*/
{
    ...
    if (PAL_MKFDecompressChunk(buf, 320 * 200, wChunkNum, gpGlobals->f.fpFBP) <=
        0) {
    ...
    if (wFade) {
        ...
        p = SDL_CreateRGBSurface(
            gpScreen->flags & ~SDL_HWSURFACE, 320, 200, 8,
        ...
        for (i = 0; i < 16; i++) {
            for (j = 0; j < 6; j++) {
                //
                // Blend the pixels in the 2 buffers, and put the result into
                // the backup buffer
                //
                for (k = rgIndex[j]; k < gpScreen->pitch * gpScreen->h;
                     k += 6) {
                    a = ((LPBYTE)(p->pixels))[k];
                ...
                if (g_wCurEffectSprite != 0) {
                    ...
                    PAL_RLEBlitToSurface(
                        PAL_SpriteGetFrame(
                            bufSprite, f % PAL_SpriteGetNumFrames(bufSprite)),
                        gpScreen, PAL_XY(0, 0));
                }
...
--*/
{
    ...
    for (l = 0; l < 220; l++) {
        ...
        if (gpGlobals->fNeedToFadeIn) {
            ...
            SDL_SetSurfacePalette(p, gpScreen->format->palette);
#else
            SDL_SetPalette(p, SDL_PHYSPAL | SDL_LOGPAL, VIDEO_GetPalette(), 0,
                           256);
#endif
        }
...
--*/
{
...
#else
    SDL_SetPalette(pUpper, SDL_PHYSPAL | SDL_LOGPAL, VIDEO_GetPalette(), 0,
                   256);
    SDL_SetPalette(pLower, SDL_PHYSPAL | SDL_LOGPAL, VIDEO_GetPalette(), 0,
                   256);
#endif
    ...
    for (i = 0; i < 400; i++) {
        ...
        // Draw the beast
        //
        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(buf, 0), gpScreen,
                             PAL_XY(0, -400 + i));
#ifdef PAL_WIN95
        PAL_RLEBlitToSurface(buf + 0x8444, gpScreen, PAL_XY(0, -200 + i));
#else
        PAL_RLEBlitToSurface(PAL_SpriteGetFrame(buf, 1), gpScreen,
                             PAL_XY(0, -200 + i));
#endif
        //
        // Draw the girl
        ...
        PAL_RLEBlitToSurface(
            PAL_SpriteGetFrame(bufGirl, (SDL_GetTicks() / 50) % 4), gpScreen,
            PAL_XY(220, yPosGirl));
        ...
        if (gpGlobals->fNeedToFadeIn) {
            ...
            SDL_SetSurfacePalette(pLower, gpScreen->format->palette);
#else
            SDL_SetPalette(pUpper, SDL_LOGPAL | SDL_PHYSPAL, VIDEO_GetPalette(),
                           0, 256);
            SDL_SetPalette(pLower, SDL_LOGPAL | SDL_PHYSPAL, VIDEO_GetPalette(),
                           0, 256);
#endif
        }
