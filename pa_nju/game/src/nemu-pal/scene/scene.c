    ...
    for (y = (sy - height - 15) / 16; y <= sy / 16; y++) {
        for (x = (sx - width / 2) / 32; x <= (sx + width / 2) / 32; x++) {
            for (i = ((x == (sx - width / 2) / 32) ? 0 : 3); i < 5; i++) {
                ...
                for (l = 0; l < 2; l++) {
                    lpTile = PAL_MapGetTileBitmap(dx, dy, dh, l,
                                                  PAL_GetCurrentMap());
                    iTileHeight = (signed char)PAL_MapGetTileHeight(
                        dx, dy, dh, l, PAL_GetCurrentMap());
...
--*/
{
    ...
    for (i = 0;
         ...
         i++) {
        LPCBITMAPRLE lpBitmap = PAL_SpriteGetFrame(
            PAL_GetPlayerSprite((BYTE)i), gpGlobals->rgParty[i].wFrame);
    ...
    for (x = 0; x < g_nSpriteToDraw - 1; x++) {
        ...
        for (y = 0; y < g_nSpriteToDraw - 1 - x; y++) {
            if (PAL_Y(g_rgSpriteToDraw[y].pos) >
...
--*/
{
    ...
    if (PAL_X(pos) < 0 || PAL_X(pos) >= 2048 || PAL_Y(pos) < 0 ||
        PAL_Y(pos) >= 2048) {
        return TRUE;
    }
    ...
    if (fCheckEventObjects) {
        ...
        //
        int i;
        for (i = gpGlobals->g.rgScene[gpGlobals->wNumScene - 1]
                     .wEventObjectIndex;
             i < gpGlobals->g.rgScene[gpGlobals->wNumScene].wEventObjectIndex;
             i++) {
...
--*/
{
    ...
    if (fWalking) {
        ...
        //
        // Player is not moved. Use the "standing" gesture instead of "walking"
        // one.
        //
...
--*/
{
    ...
    if (g_InputState.dir != kDirUnknown) {
        xOffset =
            ((g_InputState.dir == kDirWest || g_InputState.dir == kDirSouth)
                 ? -16
                 : 16);
        yOffset =
            ((g_InputState.dir == kDirWest || g_InputState.dir == kDirNorth)
                 ? -8
                 : 8);
...
--*/
{
    ...
    //
    p->x +=
        ((p->wDirection == kDirWest || p->wDirection == kDirSouth) ? -2 : 2) *
        iSpeed;
    p->y +=
        ((p->wDirection == kDirWest || p->wDirection == kDirNorth) ? -1 : 1) *
        iSpeed;
