...
//
    ...
    gpWindow = SDL_CreateWindow("Pal", SDL_WINDOWPOS_UNDEFINED,
                                SDL_WINDOWPOS_UNDEFINED, wScreenWidth,
                                wScreenHeight, SDL_WINDOW_SHOWN);
#else
    gpWindow = SDL_CreateWindow(
        "Pal", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, wScreenWidth,
        wScreenHeight, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
#endif
    ...
    if (overlay != NULL) {
        SDL_SetColorKey(overlay, SDL_RLEACCEL,
                        SDL_MapRGB(overlay->format, 255, 0, 255));
...
//
        ...
        PAL_DrawText("\xA6\x41\xA6\xB8\xAB\xF6 Back \xB5\xB2\xA7\xF4",
                     PAL_XY(30, 30), 1, FALSE, FALSE);
        gpScreen = p;
        gpBackKeyMessage =
            SDL_CreateTextureFromSurface(gpRenderer, gpScreenBak);
        SDL_FillRect(gpScreenBak, NULL, 0);
#ifdef PAL_WIN95
        fIsBig5 = FALSE;
#endif
...
//
#if defined(NDS)
    gpScreenReal =
        SDL_SetVideoMode(293, 196, 8, SDL_SWSURFACE | SDL_FULLSCREEN);
#elif defined(__SYMBIAN32__)
#ifdef __S60_5X__
    gpScreenReal = SDL_SetVideoMode(
        640, 360, 8, SDL_SWSURFACE | (fFullScreen ? SDL_FULLSCREEN : 0));
#else
    gpScreenReal = SDL_SetVideoMode(
        320, 240, 8, SDL_SWSURFACE | (fFullScreen ? SDL_FULLSCREEN : 0));
#endif
#elif defined(GEKKO)
    gpScreenReal = SDL_SetVideoMode(
...
#elif defined(PSP)
    gpScreenReal =
        SDL_SetVideoMode(320, 240, 8, SDL_SWSURFACE | SDL_FULLSCREEN);
#else
    gpScreenReal = SDL_SetVideoMode(320, 200, 8,
                                    SDL_HWSURFACE | SDL_RESIZABLE |
                                        (fFullScreen ? SDL_FULLSCREEN : 0));
#endif
    ...
    // Create the screen buffer and the backup screen buffer.
    //
    gpScreen = SDL_CreateRGBSurface(
        gpScreenReal->flags & ~SDL_HWSURFACE, 320, 200, 8,
        gpScreenReal->format->Rmask, gpScreenReal->format->Gmask,
        gpScreenReal->format->Bmask, gpScreenReal->format->Amask);

    gpScreenBak = SDL_CreateRGBSurface(
        gpScreenReal->flags & ~SDL_HWSURFACE, 320, 200, 8,
        gpScreenReal->format->Rmask, gpScreenReal->format->Gmask,
        gpScreenReal->format->Bmask, gpScreenReal->format->Amask);
...
--*/
{
    ...
    if (lpRect != NULL) {
        ...
        dstrect.y = (SHORT)((INT)(screenRealY + lpRect->y) * screenRealHeight /
                            gpScreen->h);
...
#else
        SDL_UpdateRect(gpScreenReal, dstrect.x, dstrect.y, dstrect.w,
                       dstrect.h);
#endif
    } else {
        incr_nr_draw();
        ...
        if (g_wShakeTime != 0) {
            ...
            if (g_wShakeTime & 1) {
                ...
                dstrect.y = (screenRealY + g_wShakeLevel) * screenRealHeight /
                            gpScreen->h;
            }
            ...
            SDL_RenderPresent(gpRenderer);
#else
            SDL_UpdateRect(gpScreenReal, 0, 0, gpScreenReal->w,
                           gpScreenReal->h);
#endif
            g_wShakeTime--;
        } else {
...
// TODO
    ...
    //
    if (g_wInitialWidth == 640 && g_wInitialHeight == 400 &&
        (flags & SDL_FULLSCREEN)) {
        ...
        gpScreenReal =
            SDL_SetVideoMode(g_wInitialWidth, g_wInitialHeight, 8, flags);
    }
...
/*++
  Purpose:
    ...
    Switch the screen from the backup screen buffer to the current screen
buffer. NOTE: This will destroy the backup buffer.
...
--*/
{
    ...
    for (i = 0; i < 6; i++) {
        for (j = rgIndex[i]; j < gpScreen->pitch * gpScreen->h; j += 6) {
            ((LPBYTE)(gpScreenBak->pixels))[j] =
                ((LPBYTE)(gpScreen->pixels))[j];
        }
    ...
    for (i = 0; i < 12; i++) {
        for (j = 0; j < 6; j++) {
            ...
            if (g_wShakeTime != 0) {
                ...
                dstrect.w = 320 * gpScreenReal->w / gpScreen->w;
                dstrect.h =
                    (200 - g_wShakeLevel) * screenRealHeight / gpScreen->h;

                if (g_wShakeTime & 1) {
                    srcrect.y = g_wShakeLevel;
                } else {
                    dstrect.y = (screenRealY + g_wShakeLevel) *
                                screenRealHeight / gpScreen->h;
                }
                ...
                if (g_wShakeTime & 1) {
                    dstrect.y =
                        (screenRealY + screenRealHeight - g_wShakeLevel) *
                        screenRealHeight / gpScreen->h;
                } else {
                    dstrect.y = screenRealY;
                }
                ...
                if (gpBackKeyMessage) {
                    ...
                    if (g_uiLastBackKeyTime != 0 &&
                        SDL_GetTicks() - g_uiLastBackKeyTime < 800) {
                        SDL_RenderCopy(gpRenderer, gpBackKeyMessage, NULL,
                                       NULL);
                    }
                }
#endif
