...
static SHORT
PAL_CalcMagicDamage(WORD wMagicStrength, WORD wDefense,
...
/*++
   ...
   Parameters:
     ...
     [IN]  rgwElementalResistance - inflictor's resistance to the elemental
magics.
...
--*/
{
    ...
    if (gpGlobals->g.lprgMagic[wMagicID].wElemental != 0) {
        ...
        if (wElem <= NUM_MAGIC_ELEMENTAL) {
            sDamage *=
                10 + gpGlobals->g.lprgBattleField[gpGlobals->wNumBattleField]
...
SHORT
PAL_CalcPhysicalAttackDamage(WORD wAttackStrength, WORD wDefense,
...
--*/
{
    ...
    if (gpGlobals->bBattleSpeed > 1) {
        g_Battle.flTimeChargingUnit = (g_Battle.flTimeChargingUnit /
                                       ((gpGlobals->bBattleSpeed + 1) >> 1));
    } else {
...
--*/
{
    ...
    for (i = 0; i < wDuration; i++) {
        ...
        if (wObjectID != 0) {
            ...
            {
                PAL_DrawText(PAL_GetWord(wObjectID), PAL_XY(130, 75), 15, TRUE,
                             FALSE);
            } else if ((SHORT)wObjectID < 0) {
                PAL_DrawText(PAL_GetWord(-((SHORT)wObjectID)), PAL_XY(170, 45),
                             DESCTEXT_COLOR, TRUE, FALSE);
            } else {
                PAL_DrawText(PAL_GetWord(wObjectID), PAL_XY(210, 50), 15, TRUE,
                             FALSE);
            }
        }
...
--*/
{
    ...
    for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
        ...
        if (g_Battle.rgEnemy[i].wPrevHP != g_Battle.rgEnemy[i].e.wHealth) {
            ...
            //
            sDamage =
                g_Battle.rgEnemy[i].e.wHealth - g_Battle.rgEnemy[i].wPrevHP;
            ...
            if (sDamage < 0) {
                PAL_BattleUIShowNum((WORD)(-sDamage), PAL_XY(x, y),
                                    kNumColorBlue);
            } else {
                PAL_BattleUIShowNum((WORD)(sDamage), PAL_XY(x, y),
                                    kNumColorYellow);
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
        ...
        if (g_Battle.rgPlayer[i].wPrevHP !=
            gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole]) {
        ...
        if (g_Battle.rgPlayer[i].wPrevMP !=
            gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole]) {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
            ...
            if (gpGlobals->g.PlayerRoles.rgwHP[w] <
                ...
                if (gpGlobals->g.PlayerRoles.rgwHP[w] > 0 &&
                    ...
                    if (gpGlobals->g.rgObject[wName]
                            .player.wScriptOnFriendDeath != 0) {
                        PAL_BattleDelay(10, 0, TRUE);

                        PAL_BattleMakeScene();
                        SDL_BlitSurface(g_Battle.lpSceneBuf, NULL, gpScreen,
                                        NULL);
                        VIDEO_UpdateScreen(NULL);

                        g_Battle.BattleResult = kBattleResultPause;

                        gpGlobals->g.rgObject[wName]
                            .player.wScriptOnFriendDeath = PAL_RunTriggerScript(
                            gpGlobals->g.rgObject[wName]
                                .player.wScriptOnFriendDeath,
                            w);
            ...
            if (gpGlobals->g.PlayerRoles.rgwHP[w] <
                ...
                if (gpGlobals->g.PlayerRoles.rgwHP[w] > 0 &&
                    ...
                    if (gpGlobals->rgPlayerStatus[wCover][kStatusSleep] != 0 ||
                        gpGlobals->rgPlayerStatus[wCover][kStatusParalyzed] !=
                            0 ||
                        gpGlobals->rgPlayerStatus[wCover][kStatusConfused] !=
                            0) {
                        continue;
                    }
                    ...
                    if (gpGlobals->g.rgObject[wName].player.wScriptOnDying !=
                        0) {
                        PAL_BattleDelay(10, 0, TRUE);
                        ...
                        gpGlobals->g.rgObject[wName].player.wScriptOnDying =
                            PAL_RunTriggerScript(gpGlobals->g.rgObject[wName]
                                                     .player.wScriptOnDying,
                                                 w);
            ...
            g_Battle.rgEnemy[i].e.wIdleAnimSpeed =
                gpGlobals->g
                    .lprgEnemy[gpGlobals->g
                                   .rgObject[g_Battle.rgEnemy[i].wObjectID]
                                   .enemy.wEnemyID]
                    .wIdleAnimSpeed;
        }

        if (g_Battle.rgEnemy[i].wCurrentFrame >=
            g_Battle.rgEnemy[i].e.wIdleFrames) {
            g_Battle.rgEnemy[i].wCurrentFrame = 0;
        }
    }
}
...
--*/
{
    ...
    if (g_Battle.fEnemyCleared) {
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
            ...
            if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] != 0) {
                ...
                fEnded = FALSE;
                break;
            } else if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusPuppet] !=
                       0) {
                fEnded = FALSE;
            }
        }
                ...
                g_Battle.rgEnemy[i].fDualMove =
                    (!g_Battle.rgEnemy[i].fFirstMoveDone &&
                     (g_Battle.rgEnemy[i].e.wDualMove >= 2 ||
                      (g_Battle.rgEnemy[i].e.wDualMove != 0 &&
                       RandomLong(0, 1))));
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
        ...
        case kFighterAct:
            ...
            } else if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusConfused] >
                       0) {
                g_Battle.rgPlayer[i].action.ActionType =
                    kBattleActionAttackMate;
                g_Battle.rgPlayer[i].action.flRemainingTime = 0;
            } else if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSilence] >
                           0 &&
                       g_Battle.rgPlayer[i].action.ActionType ==
                           kBattleActionMagic) {
                g_Battle.rgPlayer[i].action.flRemainingTime = 0;
    ...
    if (g_Battle.Phase == kBattlePhaseSelectAction) {
        if (g_Battle.UI.state == kBattleUIWait) {
            ...
            if (i > gpGlobals->wMaxPartyMemberIndex) {
                //
                // actions for all players are decided. fill in the action
                // queue.
                //
                ...
                for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
                    ...
                    g_Battle.ActionQueue[j].wDexterity =
                        PAL_GetEnemyDexterity(i);
                    g_Battle.ActionQueue[j].wDexterity =
                        (int)((RandomFloat(0.9, 1.1) *
                               g_Battle.ActionQueue[j].wDexterity));

                    j++;

                    if (g_Battle.rgEnemy[i].e.wDualMove * 50 +
                            RandomLong(0, 100) >
                        100) {
                        g_Battle.ActionQueue[j].fIsEnemy = TRUE;
                        g_Battle.ActionQueue[j].wIndex = i;
                ...
                for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
                    ...
                    if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] == 0 ||
                        gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSleep] >
                            0 ||
                        gpGlobals->rgPlayerStatus[wPlayerRole]
                                                 [kStatusParalyzed] > 0) {
                        //
                        // players who are unable to move should attack
                        // physically if recovered in the same turn
                        //
                        ...
                        if (gpGlobals->rgPlayerStatus[wPlayerRole]
                                                     [kStatusConfused] > 0) {
                            g_Battle.rgPlayer[i].action.ActionType =
                                kBattleActionAttack;
                            g_Battle.rgPlayer[i].state = kFighterAct;
                        ...
                        wDexterity =
                            (int)((RandomFloat(0.9, 1.1) * wDexterity));
        ...
        if (g_Battle.iCurAction >= MAX_ACTIONQUEUE_ITEMS ||
            ...
            if (g_Battle.iHidingTime == 0) {
                for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
                    ...
                    g_Battle.rgEnemy[i].wScriptOnTurnStart =
                        PAL_RunTriggerScript(
                            g_Battle.rgEnemy[i].wScriptOnTurnStart, i);
                }
            }
            ...
            if (g_Battle.ActionQueue[g_Battle.iCurAction].fIsEnemy) {
                if (g_Battle.iHidingTime == 0 && !fOnlyPuppet &&
                    g_Battle.rgEnemy[i].wObjectID != 0) {
                    g_Battle.rgEnemy[i].wScriptOnReady = PAL_RunTriggerScript(
                        g_Battle.rgEnemy[i].wScriptOnReady, i);
                ...
                if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] == 0) {
                    if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusPuppet] ==
                        0) {
                        g_Battle.rgPlayer[i].action.ActionType =
                            kBattleActionPass;
                    }
                } else if (gpGlobals->rgPlayerStatus[wPlayerRole]
                                                    [kStatusSleep] > 0 ||
                           gpGlobals->rgPlayerStatus[wPlayerRole]
                                                    [kStatusParalyzed] > 0) {
                    g_Battle.rgPlayer[i].action.ActionType = kBattleActionPass;
...
--*/
{
    ...
    } else if (g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex]
                   .action.ActionType == kBattleActionPass) {
        g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.ActionType =
            kBattleActionAttack;
    ...
    case kBattleActionMagic:
        ...
        if (gpGlobals->g.PlayerRoles
                .rgwMP[gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex]
                           .wPlayerRole] < w) {
            w = g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.wActionID;
            w = gpGlobals->g
                    .lprgMagic[gpGlobals->g.rgObject[w].magic.wMagicNumber]
                    .wType;
            if (w == kMagicTypeApplyToPlayer || w == kMagicTypeApplyToParty ||
                w == kMagicTypeTrance) {
                g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex]
                    .action.ActionType = kBattleActionDefend;
            } else {
                g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex]
                    .action.ActionType = kBattleActionAttack;
...
#ifndef PAL_CLASSIC
    if (g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.flRemainingTime <=
        0) {
        SHORT sMax = -1;
...
--*/
{
    ...
    if (sTarget != -1) {
        ...
        enemy_h = PAL_RLEGetHeight(
            PAL_SpriteGetFrame(g_Battle.rgEnemy[sTarget].lpSprite,
    ...
    index =
        gpGlobals->g
            .rgwBattleEffectIndex[PAL_GetPlayerBattleSprite(wPlayerRole)][1];
    index *= 3;
...
--*/
{
    ...
    if (!fSummon) {
        ...
        index = gpGlobals->g.rgwBattleEffectIndex[PAL_GetPlayerBattleSprite(
            wPlayerRole)][0];
        index *= 10;
        index += 15;
#ifdef PAL_WIN95
        ...
        for (i = 0; i < 10; i++) {
            LPCBITMAPRLE b =
                PAL_SpriteGetFrame(g_Battle.lpEffectSprite, index++);
...
--*/
{
    ...
    PAL_MKFDecompressChunk((LPBYTE)lpSpriteEffect, l, iEffectNum,
                           gpGlobals->f.fpFIRE);
        ...
        if (gpGlobals->g.lprgMagic[iMagicNum].wType == kMagicTypeApplyToParty) {
            ...
            for (l = 0; l <= gpGlobals->wMaxPartyMemberIndex; l++) {
                ...
                PAL_RLEBlitToSurface(b, gpScreen,
                                     PAL_XY(x - PAL_RLEGetWidth(b) / 2,
                                            y - PAL_RLEGetHeight(b)));
            }
        } else if (gpGlobals->g.lprgMagic[iMagicNum].wType ==
                   kMagicTypeApplyToPlayer) {
            assert(sTarget != -1);
            ...
            if (sTarget > 0 && g_Battle.iHidingTime == 0) {
                if (gpGlobals->rgPlayerStatus[gpGlobals->rgParty[sTarget - 1]
                                                  .wPlayerRole]
                                             [kStatusConfused] == 0) {
                    LPCBITMAPRLE p = PAL_SpriteGetFrame(
                        g_Battle.rgPlayer[sTarget - 1].lpSprite,
            ...
            for (k = 0; k < 3; k++) {
                ...
                if (i == l - 1 && gpGlobals->wScreenWave < 9 &&
                    ...
                    PAL_RLEBlitToSurface(b, g_Battle.lpBackground,
                       ...
                       kMagicTypeAttackWhole ||
                   gpGlobals->g.lprgMagic[iMagicNum].wType ==
                       kMagicTypeAttackField) {
            assert(sTarget == -1);

            if (gpGlobals->g.lprgMagic[iMagicNum].wType ==
                kMagicTypeAttackWhole) {
                x = 120;
                y = 100;
            } else {
...
static VOID PAL_BattleShowPlayerSummonMagicAnim(WORD wPlayerIndex,
                                                WORD wObjectID)
/*++
  Purpose:
...
--*/
{
    ...
    // TODO: There is still something missing here compared to the original
    // game.
    //
    while (g_Battle.iSummonFrame <
           PAL_SpriteGetNumFrames(g_Battle.lpSummonSprite) - 1) {
        //
        // Wait for the time of one frame. Accept input here.
        //
        PAL_ProcessEvent();
...
--*/
{
    ...
    case kBattleActionMagic:
        ...
        for (i = 0; i < MAX_PLAYER_MAGICS; i++) {
            if (gpGlobals->g.PlayerRoles.rgwMagic[i][wPlayerRole] ==
                wObjectID) {
        ...
        //
        if (gpGlobals->g.rgObject[wObjectID].magic.wFlags &
    ...
    if (fToEnemy && g_Battle.rgPlayer[wPlayerIndex].action.sTarget >= 0) {
        if (g_Battle.rgEnemy[g_Battle.rgPlayer[wPlayerIndex].action.sTarget]
                .wObjectID == 0) {
            g_Battle.rgPlayer[wPlayerIndex].action.sTarget =
                PAL_BattleSelectAutoTarget();
...
--*/
{
    ...
    case kBattleActionAttack:
        if (sTarget != -1) {
            ...
            //
            for (t = 0;
                 t < (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusDualAttack]
                          ? 2
                          : 1);
                 t++) {
                ...
                if (RandomLong(0, 5) == 0 ||
                    gpGlobals->rgPlayerStatus[wPlayerRole][kStatusBravery] >
                        0) {
                    //
                    // Critical Hit
                    //
    ...
    case kBattleActionAttackMate:
        ...
        if (i <= gpGlobals->wMaxPartyMemberIndex) {
            ...
            if (gpGlobals
                    ->rgPlayerStatus[gpGlobals->rgParty[sTarget].wPlayerRole]
                                    [kStatusProtect] > 0) {
                sDamage /= 2;
            }
            ...
            gpGlobals->g.PlayerRoles
                .rgwHP[gpGlobals->rgParty[sTarget].wPlayerRole] -= sDamage;
    ...
    case kBattleActionCoopMagic:
        ...
        if (gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeSummon) {
            ...
            for (i = 1; i <= 6; i++) {
                ...
                //
                x = PAL_X(g_Battle.rgPlayer[wPlayerIndex].posOriginal) *
                    (6 - i);
                y = PAL_Y(g_Battle.rgPlayer[wPlayerIndex].posOriginal) *
                    (6 - i);
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
            ...
            if ((SHORT)(gpGlobals->g.PlayerRoles
                            .rgwHP[gpGlobals->rgParty[i].wPlayerRole]) <= 0) {
                gpGlobals->g.PlayerRoles
                    .rgwHP[gpGlobals->rgParty[i].wPlayerRole] = 1;
            }
...
//
        ...
        for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
            str +=
                PAL_GetPlayerAttackStrength(gpGlobals->rgParty[i].wPlayerRole);
            str +=
                PAL_GetPlayerMagicStrength(gpGlobals->rgParty[i].wPlayerRole);
        }
    ...
    case kBattleActionMagic:
        ...
        if (gpGlobals->g.lprgMagic[wMagicNum].wType ==
                kMagicTypeApplyToPlayer ||
            ...
            if (g_Battle.rgPlayer[wPlayerIndex].action.sTarget != -1) {
                w = gpGlobals
                        ->rgParty[g_Battle.rgPlayer[wPlayerIndex]
                                      .action.sTarget]
                        .wPlayerRole;
            } else if (gpGlobals->g.lprgMagic[wMagicNum].wType ==
                       kMagicTypeTrance) {
                w = wPlayerRole;
            }
            ...
            if (g_fScriptSuccess) {
                PAL_BattleShowPlayerDefMagicAnim(wPlayerIndex, wObject,
                                                 sTarget);
            ...
            if (g_fScriptSuccess) {
...
#ifdef PAL_WIN95
                    PAL_BattleShowPlayerOffMagicAnim(wPlayerIndex, wObject,
                                                     sTarget, FALSE);
#else
                    PAL_BattleShowPlayerOffMagicAnim(wPlayerIndex, wObject,
                                                     sTarget);
                ...
                // Inflict damage to enemies
                //
                if ((SHORT)(gpGlobals->g.lprgMagic[wMagicNum].wBaseDamage) >
                    0) {
                    if (sTarget == -1) {
                        //
                        // Attack all enemies
                        ...
                        for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
                            ...
                            sDamage = PAL_CalcMagicDamage(
                                str, def, g_Battle.rgEnemy[i].e.wElemResistance,
                                g_Battle.rgEnemy[i].e.wPoisonResistance,
                                wObject);
                        ...
                        sDamage = PAL_CalcMagicDamage(
                            str, def,
                            g_Battle.rgEnemy[sTarget].e.wElemResistance,
                            g_Battle.rgEnemy[sTarget].e.wPoisonResistance,
                            wObject);
    ...
    case kBattleActionThrowItem:
        ...
        for (i = 0; i < 4; i++) {
            g_Battle.rgPlayer[wPlayerIndex].pos = PAL_XY(
                PAL_X(g_Battle.rgPlayer[wPlayerIndex].pos) - (4 - i),
                PAL_Y(g_Battle.rgPlayer[wPlayerIndex].pos) - (4 - i) / 2);
    ...
    if (fCheckPoison) {
        ...
        for (i = 0; i < MAX_POISONS; i++) {
            ...
            if (wObject != 0) {
                fPoisoned = TRUE;
                gpGlobals->rgPoisonStatus[i][wPlayerIndex]
...
--*/
{
    ...
    while (gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[i].wPlayerRole] ==
           0) {
        i = RandomLong(0, gpGlobals->wMaxPartyMemberIndex);
    }
...
--*/
{
    ...
    if (g_Battle.rgEnemy[wEnemyIndex].rgwStatus[kStatusSleep] > 0 ||
        ...
        for (i = 0; i < g_Battle.rgEnemy[wEnemyIndex].e.wMagicFrames; i++) {
            ...
            PAL_BattleDelay(g_Battle.rgEnemy[wEnemyIndex].e.wActWaitFrames, 0,
                            FALSE);
        }
        ...
        if (gpGlobals->g.lprgMagic[wMagicNum].wSoundDelay == 0) {
            for (i = 0; i <= g_Battle.rgEnemy[wEnemyIndex].e.wAttackFrames;
                 i++) {
                g_Battle.rgEnemy[wEnemyIndex].wCurrentFrame =
                    i - 1 + g_Battle.rgEnemy[wEnemyIndex].e.wIdleFrames +
                    g_Battle.rgEnemy[wEnemyIndex].e.wMagicFrames;
                PAL_BattleDelay(g_Battle.rgEnemy[wEnemyIndex].e.wActWaitFrames,
                                0, FALSE);
            }
        }
        ...
        if ((SHORT)(gpGlobals->g.lprgMagic[wMagicNum].wBaseDamage) > 0) {
            if (sTarget == -1) {
                ...
                for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
                    ...
                    sDamage /=
                        ((g_Battle.rgPlayer[i].fDefending ? 2 : 1) *
                         ((gpGlobals->rgPlayerStatus[w][kStatusProtect] > 0)
                              ? 2
                              : 1)) +
                ...
                for (x = 0; x < NUM_MAGIC_ELEMENTAL; x++) {
                    rgwElementalResistance[x] =
                        5 +
                        PAL_GetPlayerElementalResistance(wPlayerRole, x) / 20;
                }

                sDamage = PAL_CalcMagicDamage(
                    str, def, rgwElementalResistance,
                    5 + PAL_GetPlayerPoisonResistance(wPlayerRole) / 20,
                    wMagic);

                sDamage /=
                    ((g_Battle.rgPlayer[sTarget].fDefending ? 2 : 1) *
                     ((gpGlobals->rgPlayerStatus[wPlayerRole][kStatusProtect] >
                       0)
                          ? 2
                          : 1)) +
                    (fAutoDefend ? 1 : 0);
                ...
                if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] == 0) {
                    SOUND_Play(
                        gpGlobals->g.PlayerRoles.rgwDeathSound[wPlayerRole]);
                }
            }
        }
        ...
        for (i = 0; i < 5; i++) {
            if (sTarget == -1) {
                ...
                if (i > 0) {
                    g_Battle.rgPlayer[sTarget].pos = PAL_XY(
                        PAL_X(g_Battle.rgPlayer[sTarget].pos) + (8 >> i),
        ...
        g_Battle.rgEnemy[wEnemyIndex].pos =
            g_Battle.rgEnemy[wEnemyIndex].posOriginal;
        ...
        if ((PAL_IsPlayerDying(wPlayerRole) ||
            ...
            if (iCoverIndex != -1) {
                if (PAL_IsPlayerDying(
                        gpGlobals->rgParty[iCoverIndex].wPlayerRole) ||
                    gpGlobals->rgPlayerStatus[gpGlobals->rgParty[iCoverIndex]
                                                  .wPlayerRole]
                                             [kStatusConfused] > 0 ||
                    gpGlobals->rgPlayerStatus[gpGlobals->rgParty[iCoverIndex]
                                                  .wPlayerRole][kStatusSleep] >
                        0 ||
                    gpGlobals->rgPlayerStatus[gpGlobals->rgParty[iCoverIndex]
                                                  .wPlayerRole]
                                             [kStatusParalyzed] > 0) {
        ...
        if (!fAutoDefend) {
            ...
            sDamage =
                PAL_CalcPhysicalAttackDamage(str + RandomLong(0, 2), def, 2);
        ...
        if (iCoverIndex == -1 && !fAutoDefend &&
            g_Battle.rgEnemy[wEnemyIndex].e.wAttackEquivItemRate >=
                RandomLong(1, 10)) {
...
--*/
{
    ...
    if (gpGlobals->g.rgObject[wMagicObjectID].magic.wFlags &
        kMagicFlagApplyToAll) {
        sTarget = -1;
    } else if (sTarget == -1) {
...
//
    ...
    if (gpGlobals->g
                .lprgMagic[gpGlobals->g.rgObject[wMagicObjectID]
                               .magic.wMagicNumber]
                .wBaseDamage > 0 ||
        wBaseDamage > 0) {
        if (sTarget == -1) {
            ...
            for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++) {
                ...
                sDamage = PAL_CalcMagicDamage(
                    wBaseDamage, (WORD)def,
                    g_Battle.rgEnemy[i].e.wElemResistance,
