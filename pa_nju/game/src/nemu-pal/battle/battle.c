...
--*/
{
    ...
    if (g_Battle.lpSummonSprite != NULL) {
        ...
        pos = PAL_XY(PAL_X(g_Battle.posSummon) -
                         PAL_RLEGetWidth(PAL_SpriteGetFrame(
                             g_Battle.lpSummonSprite, g_Battle.iSummonFrame)) /
                             2,
        ...
        for (i = gpGlobals->wMaxPartyMemberIndex; i >= 0; i--) {
            if (gpGlobals->rgPlayerStatus[gpGlobals->rgParty[i].wPlayerRole]
                ...
                //
                pos =
                    PAL_XY(PAL_X(g_Battle.rgPlayer[i].pos),
                           PAL_Y(g_Battle.rgPlayer[i].pos) + RandomLong(-1, 1));
                pos = PAL_XY(
                    PAL_X(pos) - PAL_RLEGetWidth(PAL_SpriteGetFrame(
                                     g_Battle.rgPlayer[i].lpSprite,
                                     g_Battle.rgPlayer[i].wCurrentFrame)) /
                                     2,
                    PAL_Y(pos) - PAL_RLEGetHeight(PAL_SpriteGetFrame(
                ...
                if (g_Battle.rgPlayer[i].iColorShift != 0) {
                    PAL_RLEBlitWithColorShift(
                        PAL_SpriteGetFrame(g_Battle.rgPlayer[i].lpSprite,
                                           g_Battle.rgPlayer[i].wCurrentFrame),
                        g_Battle.lpSceneBuf, pos,
                        g_Battle.rgPlayer[i].iColorShift);
                } else if (g_Battle.iHidingTime == 0) {
                    PAL_RLEBlitToSurface(
                        PAL_SpriteGetFrame(g_Battle.rgPlayer[i].lpSprite,
                                           g_Battle.rgPlayer[i].wCurrentFrame),
...
--*/
{
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
        ...
        PAL_MKFDecompressChunk(g_Battle.rgPlayer[i].lpSprite, l, s,
                               gpGlobals->f.fpF);
...
--*/
{
    ...
    if (g_Battle.lpBackground == NULL) {
        TerminateOnError(
            "PAL_LoadBattleBackground(): failed to create surface!");
    }
#if SDL_VERSION_ATLEAST(2, 0, 0)
    SDL_SetSurfacePalette(g_Battle.lpBackground, gpScreen->format->palette);
#else
    SDL_SetPalette(g_Battle.lpBackground, SDL_PHYSPAL | SDL_LOGPAL,
...
--*/
{
    ...
    if (g_Battle.iExpGained > 0) {
        ...
        PAL_DrawText(PAL_GetWord(BATTLEWIN_GETEXP_LABEL), PAL_XY(95, 70), 0,
                     FALSE, FALSE);
        PAL_DrawText(PAL_GetWord(BATTLEWIN_BEATENEMY_LABEL), PAL_XY(77, 115), 0,
                     FALSE, FALSE);
        PAL_DrawText(PAL_GetWord(BATTLEWIN_DOLLAR_LABEL), PAL_XY(197, 115), 0,
                     FALSE, FALSE);

        PAL_DrawNumber(g_Battle.iExpGained, 5, PAL_XY(182, 74), kNumColorYellow,
                       kNumAlignRight);
        PAL_DrawNumber(g_Battle.iCashGained, 5, PAL_XY(162, 119),
                       kNumColorYellow, kNumAlignMid);
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
        ...
        while (
            dwExp >=
            gpGlobals->g.rgLevelUpExp[gpGlobals->g.PlayerRoles.rgwLevel[w]]) {
            dwExp -=
                gpGlobals->g.rgLevelUpExp[gpGlobals->g.PlayerRoles.rgwLevel[w]];
        ...
        if (fLevelUp) {
            ...
            PAL_DrawText(PAL_GetWord(STATUS_LABEL_LEVEL),
                         PAL_XY(110 + 16 * 3, 10), 0, FALSE, FALSE);
            PAL_DrawText(PAL_GetWord(BATTLEWIN_LEVELUP_LABEL),
                         PAL_XY(110 + 16 * 5, 10), 0, FALSE, FALSE);

            for (j = 0; j < 8; j++) {
                PAL_RLEBlitToSurface(
                    PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_ARROW), gpScreen,
                    PAL_XY(183, 48 + 18 * j));
            }
            ...
            PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwLevel[w], 4,
                           PAL_XY(195, 47), kNumColorYellow, kNumAlignRight);
            ...
            PAL_RLEBlitToSurface(
                PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
                PAL_XY(156, 66));
            PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwHP[w], 4,
                           PAL_XY(195, 64), kNumColorYellow, kNumAlignRight);
            PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxHP[w], 4,
                           PAL_XY(216, 68), kNumColorBlue, kNumAlignRight);
            PAL_RLEBlitToSurface(
                PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
                PAL_XY(218, 66));
            ...
            PAL_RLEBlitToSurface(
                PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
                PAL_XY(156, 84));
            PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMP[w], 4,
                           PAL_XY(195, 82), kNumColorYellow, kNumAlignRight);
            PAL_DrawNumber(gpGlobals->g.PlayerRoles.rgwMaxMP[w], 4,
                           PAL_XY(216, 86), kNumColorBlue, kNumAlignRight);
            PAL_RLEBlitToSurface(
                PAL_SpriteGetFrame(gpSpriteUI, SPRITENUM_SLASH), gpScreen,
                PAL_XY(218, 84));
            ...
            PAL_DrawNumber(OrigPlayerRoles.rgwAttackStrength[w] +
                           ...
                           4, PAL_XY(133, 101), kNumColorYellow,
                           kNumAlignRight);
            ...
            PAL_DrawNumber(OrigPlayerRoles.rgwMagicStrength[w] +
                           ...
                           4, PAL_XY(133, 119), kNumColorYellow,
                           kNumAlignRight);
            PAL_DrawNumber(PAL_GetPlayerMagicStrength(w), 4, PAL_XY(195, 119),
                           kNumColorYellow, kNumAlignRight);

            PAL_DrawNumber(
                OrigPlayerRoles.rgwDefense[w] + PAL_GetPlayerDefense(w) -
            ...
            PAL_DrawNumber(
                OrigPlayerRoles.rgwDexterity[w] + PAL_GetPlayerDexterity(w) -
            ...
            PAL_DrawNumber(
                OrigPlayerRoles.rgwFleeRate[w] + PAL_GetPlayerFleeRate(w) -
    ...
    {                                                                          \
        ...
        if (gpGlobals->Exp.expname[w].wLevel > MAX_LEVELS) {                   \
            gpGlobals->Exp.expname[w].wLevel = MAX_LEVELS;                     \
                                                                               ...
                                                                               \
        while (dwExp >=                                                        \
               gpGlobals->g.rgLevelUpExp[gpGlobals->Exp.expname[w].wLevel]) {  \
            dwExp -=                                                           \
                gpGlobals->g.rgLevelUpExp[gpGlobals->Exp.expname[w].wLevel];   \
        ...
        if (gpGlobals->g.PlayerRoles.statname[w] !=                            \
            ...
            PAL_DrawText(PAL_GetWord(label), PAL_XY(143, 70), 0, FALSE,        \
                         FALSE);                                               \
            PAL_DrawText(PAL_GetWord(BATTLEWIN_LEVELUP_LABEL),                 \
                         PAL_XY(175, 70), 0, FALSE, FALSE);                    \
            PAL_DrawNumber(gpGlobals->g.PlayerRoles.statname[w] -              \
                               OrigPlayerRoles.statname[w],                    \
                           5, PAL_XY(188, 74), kNumColorYellow,                \
                           kNumAlignRight);                                    \
            VIDEO_UpdateScreen(&rect);                                         \
            PAL_WaitForKey(3000);                                              \
        }                                                                      \
            ...
            CHECK_HIDDEN_EXP(rgAttackExp, rgwAttackStrength,
                             STATUS_LABEL_ATTACKPOWER);
            CHECK_HIDDEN_EXP(rgMagicPowerExp, rgwMagicStrength,
                             STATUS_LABEL_MAGICPOWER);
            CHECK_HIDDEN_EXP(rgDefenseExp, rgwDefense, STATUS_LABEL_RESISTANCE);
            CHECK_HIDDEN_EXP(rgDexterityExp, rgwDexterity,
...
#undef CHECK_HIDDEN_EXP
        ...
        while (j < gpGlobals->g.nLevelUpMagic) {
            ...
            if (PAL_AddMagic(w, gpGlobals->g.lprgLevelUpMagic[j].m[w].wMagic)) {
                ...
                PAL_DrawText(
                    PAL_GetWord(gpGlobals->g.lprgLevelUpMagic[j].m[w].wMagic),
    ...
    for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++) {
        ...
        if (gpGlobals->g.PlayerRoles.rgwHP[w] == 0) {
            ...
            float f = (gpGlobals->g
                           .rgLevelUpExp[gpGlobals->g.PlayerRoles.rgwLevel[w]] /
                       5.0f) /
                      g_Battle.iExpGained;
...
--*/
{
    ...
    while (f) {
        ...
        for (j = 0; j <= g_Battle.wMaxEnemyIndex; j++) {
            ...
            w = PAL_RLEGetWidth(
                PAL_SpriteGetFrame(g_Battle.rgEnemy[j].lpSprite, 0));
...
#else
    SDL_SetPalette(g_Battle.lpSceneBuf, SDL_PHYSPAL | SDL_LOGPAL,
                   VIDEO_GetPalette(), 0, 256);
#endif
